...
if(isset($_GET[enviar_fact]))
	...
	$revisarmonto=mysql_query("select monto_fact from factura_trab");
	$revisarfecha=mysql_query("select fecha_fact from factura_trab");
	$cantidadtasarevision=mysql_num_rows($revisarsiencuentra);

	for($j=1;$j<=$revisionfechamonto;$j++)
	{
		$fechafact=mysql_fetch_array($revisarfecha);
		$montofact=mysql_fetch_array($revisarmonto);
		$fec_fact=$campo_fact['fecha_fact'];
		$mon_fact=$campo_fact['monto_fact'];
	}
	...
	else
	{
		?>
		<script>
		alert("La factura <?php echo $_GET[observacion_fact] ?> codigo <?php echo $_GET[codigo_fact] ?> se registro la fecha <?php echo $fec_fact ?> por un monto de <?php echo $mon_fact ?>  ");
        </script>
		<?php
	}
...
<fieldset id="cuerpofacturatrabajador" style="background-color:#FFF">
 ...
 <table width="50%" border="0" id="cuadro">
  ...
  <tr>
    ...
    <td>
      <input type="date" name="fecha_fact" id="fecha_trab" style="width:98%"  /></td>
  </tr>
  <tr>
 ...
 <tr>
    ...
    <td>
      <select name="gasto_fact" id="gasto_fact" style="width:99%">
